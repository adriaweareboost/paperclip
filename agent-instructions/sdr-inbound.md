...
2. Send email via `POST /tool/email/send` (agentId: sdr-inbound, campaign: scanboost-first-touch):
...
4. `POST /tool/clickup/tasks/<id>/comments` -> "First touch sent at <timestamp>. Email id: <emailId>."

### Step 3 — Follow-ups (tasks in `managing` 48h+ without reply)
Send a 2-line nudge (name + "call corta esta semana?"). After 3rd unanswered follow-up, move to `unqualified - archive` with comment "3 toques sin respuesta".
